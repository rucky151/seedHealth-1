...
#' Create a matrix with output from multiple simulations for a single parameter
#' combination
#'
#' This function creates a matrix for output from multiple simulations for a
#' single parameter combination across nseasons
#'
#' The truncated random normal variables were generated by
#' \code{\link{altruncnorm}}. Refer \code{\link{onesim}} and
#' \code{\link{multipar}}for relevant information.
#'
...
multisim <- function(pHSinit = 0.8, Kx = 100, betax = 0.02, wxtnormm = 0.8, 
...
#---------------------------------------------- end
  ...
  outfsum <- as.data.frame(matrix(data=-999, nrow=5, ncol=12, 
                                  dimnames = list(c('mean','median','var','q0.05','q0.95'), 
                                                  c('fHP', 'fDP', 'fHS', 'fDS', 'fpHS', 'fpDS',  'HPtrans', 'pHStrans', 'HPpseas', 'pHSpseas', 'fYld', 'fYL'))))
